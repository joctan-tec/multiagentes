...
const fetchChatGPTResponse = async (message) => {
  try {
    const response = await fetch("http://localhost:30500/chatgpt", {
      method: "POST",
      headers: {
    ...
    if (message.trim() !== "") {
      ...
      await fetchChatGPTResponse(message)
        .then((chatgptResponse) => {
          ...
          // Primero convierte los caracteres escapados a su representación real
          const cleanText = chatgptResponse
            .replace(/\\n/g, '\n')
            .replace(/\\\*/g, '*')  // si aparecen escapados
            .replace(/\\_/g, '_');

          // Ahora pásalo a marked
          loadingBubble.innerHTML = marked.parse(cleanText);
          loadingBubble.classList.remove("loading-animation");
          console.log("Respuesta de ChatGPT:", cleanText);
          messagesContainer.scrollTop = messagesContainer.scrollHeight;
